...
from load_data import load_simulated_data, load_error_data
...
def example_inference(n_ahead=100):
    ...
    for i in range(10):
        save_plot_inference(simulated_dataset, 'Simulated First-Principles', f'plots/simulated{i}.png', model_simulated, model_error)
        save_plot_inference(hardware_dataset, 'Actual Hardware Data', f'plots/hardware{i}.png', model_simulated, model_error)
...
def save_plot_inference(dataset, suptitle, filename, model_simulated, model_error):
    ...
    with torch.no_grad():
        for i in range(len(deltas)):
            ...
            d_both = model_simulated(actions[i].reshape(1,-1), s_both.reshape(1,-1)) + model_error(actions[i].reshape(1,-1), s_both.reshape(1,-1))
